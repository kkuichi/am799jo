"""
ALBERT fine-tuning for toxicity detection
(Jigsaw Toxic Comment Classification Challenge)

Bachelor's thesis:
Využitie transformerov v detekcii toxicity na sociálnych sieťach

Fourth experiment — ALBERT (A Lite BERT, Lan et al., 2019).
Key differences from BERT:
  1. Cross-layer parameter sharing — all layers share parameters
  2. Factorized embedding parameterization — embedding matrix is split into two smaller ones
  3. Sentence Order Prediction (SOP) instead of Next Sentence Prediction (NSP)

Result: ALBERT-base-v2 has only ~12M parameters (vs 110M for BERT),
but inference computational cost is comparable to BERT,
because layers are still evaluated sequentially (they only share weights).

Hyperparameters are identical to BERT/RoBERTa/DistilBERT for a fair comparison.
...
class Config:
    ...
    OUTPUT_DIR   = "./albert_toxicity_model"
...
# 2. Dataset
# --------------------------------------------------
# ALBERT (like BERT) uses token_type_ids.
...
if __name__ == "__main__":
    ...
    tokenizer = AlbertTokenizer.from_pretrained(cfg.MODEL_NAME)
    ...
    print(f"(compare: BERT 110M, RoBERTa 125M, DistilBERT 67M)")
    ...
    # Training
    print("\n" + "="*60)
    print("STARTING ALBERT TRAINING (with AMP)")
    print("="*60)
    ...
    best_model = AlbertForSequenceClassification.from_pretrained(cfg.OUTPUT_DIR)

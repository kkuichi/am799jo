"""
RoBERTa fine-tuning for toxicity detection
(Jigsaw Toxic Comment Classification Challenge)

Bachelor's thesis:
Využitie transformerov v detekcii toxicity na sociálnych sieťach

Parallel experiment to BERT for architecture comparison.
The hyperparameters are identical to train_bert.py, which is important
for a fair comparison.

Automatic Mixed Precision (AMP) is used to accelerate training by approximately
60-80%. Its influence on quality is minimal, usually affecting only the third
...
class Config:
    ...
    MAX_LEN       = 256
    BATCH_SIZE    = 16        # Can be increased to 32 if VRAM allows it with fp16.
    EPOCHS        = 3
    LEARNING_RATE = 2e-5
    WARMUP_RATIO  = 0.1
    WEIGHT_DECAY  = 0.01
...
# 2. Dataset
# --------------------------------------------------
# RoBERTa does not use token_type_ids, because NSP was not used during pretraining.
...
if __name__ == "__main__":
    ...
    # Training
    print("\n" + "="*60)
    print("STARTING ROBERTA TRAINING (with AMP)")
    print("="*60)
    ...
    best_model = RobertaForSequenceClassification.from_pretrained(cfg.OUTPUT_DIR)
